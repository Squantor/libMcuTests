...
MINUNIT_ADD(RP2040LLUartSetup, RP2040LLSetupUart, Rp2040Teardown) {
  ...
  MINUNIT_CHECK(uart_polled_peripheral.Setup(7800000) == 7500000);
  MINUNIT_CHECK(uart0Registers->UARTIBRD == 1);
  MINUNIT_CHECK(uart_polled_peripheral.Setup(100) == 114);
  MINUNIT_CHECK(uart0Registers->UARTIBRD == 65535);
  // test all common baud rates and their divisors
  MINUNIT_CHECK(uart_polled_peripheral.Setup(300) == 300);
  // check control registers and divisors and such
  MINUNIT_CHECK(uart_polled_peripheral.Setup(1200) == 1200);
  // check control registers and divisors and such
  MINUNIT_CHECK(uart_polled_peripheral.Setup(4800) == 4800);
  // check control registers and divisors and such
  MINUNIT_CHECK(uart_polled_peripheral.Setup(9600) == 9600);
  // check control registers and divisors and such
  MINUNIT_CHECK(uart_polled_peripheral.Setup(38400) == 38400);
  // check control registers and divisors and such
  MINUNIT_CHECK(uart_polled_peripheral.Setup(115200) == 115190);
  // check control registers and divisors and such
  MINUNIT_CHECK(uart_polled_peripheral.Setup(1000000) == 1000000);
  // check control registers and divisors and such
}
...
MINUNIT_ADD(RP2040LLUartComms, RP2040LLSetupUart, Rp2040Teardown) {
  ...
  MINUNIT_CHECK(uart_polled_peripheral.Setup(9600) == 9600);
  MINUNIT_CHECK(uart0Registers->UARTFR & hardware::UARTFR::TXFE_FLAG);
  MINUNIT_CHECK(uart0Registers->UARTFR & hardware::UARTFR::RXFE_FLAG);
  MINUNIT_CHECK(uart_polled_peripheral.Receive(receiveDataSingle, 100) == libmcu::Results::Timeout);
  uart_polled_peripheral.Transmit(singleData);
  MINUNIT_CHECK(uart0Registers->UARTFR & hardware::UARTFR::TXFE_FLAG);
  libmcu::Delay(1000000);  // wait until data has been received
  MINUNIT_CHECK(
    !(uart0Registers->UARTFR & hardware::UARTFR::RXFE_FLAG));  // we looped TX and RX so RX FIFO should also not be empty
  MINUNIT_CHECK(uart_polled_peripheral.Receive(receiveDataSingle, 100) == libmcu::Results::NoError);
  MINUNIT_CHECK(receiveDataSingle[0] == 0x5A);
  MINUNIT_CHECK(uart0Registers->UARTFR & hardware::UARTFR::TXFE_FLAG);
  MINUNIT_CHECK(uart0Registers->UARTFR & hardware::UARTFR::RXFE_FLAG);
  uart_polled_peripheral.Transmit(multiData);
  MINUNIT_CHECK(uart_polled_peripheral.Receive(receiveDataMulti, 10000) == libmcu::Results::NoError);
  MINUNIT_CHECK(uart0Registers->UARTFR & hardware::UARTFR::TXFE_FLAG);
  MINUNIT_CHECK(uart0Registers->UARTFR & hardware::UARTFR::RXFE_FLAG);
  MINUNIT_CHECK(receiveDataMulti[0] == 0x01);
  MINUNIT_CHECK(receiveDataMulti[1] == 0x23);
  MINUNIT_CHECK(receiveDataMulti[2] == 0x45);
  MINUNIT_CHECK(receiveDataMulti[3] == 0x67);
  MINUNIT_CHECK(receiveDataMulti[4] == 0xA5);
}

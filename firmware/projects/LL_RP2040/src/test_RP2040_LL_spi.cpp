...
MINUNIT_ADD(RP2040spiSetup, RP2040SetupSPI, Rp2040Teardown) {
  ...
  MINUNIT_CHECK(bitRate == 1'000'000);
  MINUNIT_CHECK(spi0Registers->SSPCR0 == 0x3C00);
  bitRate = spi_polled_peripheral.SetupMaster(1'234'567, libmcull::spi::Waveforms::Cpha1Cpol1);
  MINUNIT_CHECK(bitRate == 1'250'000);
  MINUNIT_CHECK(spi0Registers->SSPCR0 == 0x30C0);
}
...
MINUNIT_ADD(RP2040spiComms, RP2040SetupSPI, Rp2040Teardown) {
  ...
  MINUNIT_CHECK(receiveBuffer[0] == 0x234);
  spi_polled_peripheral.Transceive(MultiTransmitBuffer, receiveBuffer, 8);
  MINUNIT_CHECK(receiveBuffer[0] == 0x23);
  MINUNIT_CHECK(receiveBuffer[1] == 0x67);
  MINUNIT_CHECK(receiveBuffer[2] == 0xAB);
  MINUNIT_CHECK(receiveBuffer[3] == 0xEF);
  spi_polled_peripheral.Transceive(MultiTransmitBuffer, receiveBuffer, 16);
  MINUNIT_CHECK(receiveBuffer[0] == 0x0123);
  MINUNIT_CHECK(receiveBuffer[1] == 0x4567);
  MINUNIT_CHECK(receiveBuffer[2] == 0x89AB);
  MINUNIT_CHECK(receiveBuffer[3] == 0xCDEF);
}
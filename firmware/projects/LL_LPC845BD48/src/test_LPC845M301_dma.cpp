...
MINUNIT_ADD(LPC845M301DH20DmaInit, LPC845M301SetupDma, LPC845M301Teardown) {
  ...
  for (auto &element : dma_peripheral.GetDescriptorTable()) {
    ...
    MINUNIT_CHECK(element.destination_end_address == 0);
    MINUNIT_CHECK(element.next_descriptor == nullptr);
  }
}
...
MINUNIT_ADD(LPC845M301DH20DmaMemToMemTransfer, LPC845M301SetupDma, LPC845M301Teardown) {
  ...
  MINUNIT_CHECK(table[12].source_end_address == reinterpret_cast<std::uint32_t>(inputBuffer.data() + inputBuffer.size() - 1));
  MINUNIT_CHECK(table[12].destination_end_address == reinterpret_cast<std::uint32_t>(outputBuffer.data() + inputBuffer.size() - 1));
  MINUNIT_CHECK(table[12].next_descriptor == nullptr);
  ...
  MINUNIT_CHECK(wait_count < 1000);
  MINUNIT_CHECK(dma_peripheral.IsChannelActive(SoftwareDescriptors::Channel12) == false);
  MINUNIT_CHECK(memcmp(inputBuffer.data(), outputBuffer.data(), inputBuffer.size()) == 0);
}
...
MINUNIT_ADD(LPC845M301DH20DmaMemToPeriTransfer, LPC845M301SetupDma, LPC845M301Teardown) {
  ...
  MINUNIT_CHECK((status & UartStatuses::TxIdle) != 0);
  MINUNIT_CHECK((status & UartStatuses::RxIdle) != 0);
  ...
  // check transfer results
  MINUNIT_CHECK(wait_count < 2000);
  MINUNIT_CHECK(dma_peripheral.IsChannelActive(HardwareDescriptors::Usart0TxDma) == false);
  // DMA seems to wrap around to 1024 when completing its transfer
  MINUNIT_CHECK(XFERCFG::GetXFERCOUNT(dma_registers->CHANNEL[1].XFERCFG) == 1024);
}
...
MINUNIT_ADD(LPC845M301DH20DmaMemToPeriAndPeriToMemTransfer, LPC845M301SetupDma, LPC845M301Teardown) {
  ...
  MINUNIT_CHECK(wait_count < 2000);
  MINUNIT_CHECK(dma_peripheral.IsChannelActive(HardwareDescriptors::Usart0TxDma) == false);
  MINUNIT_CHECK(dma_peripheral.IsChannelActive(HardwareDescriptors::Usart0RxDma) == false);
  // DMA seems to wrap around to 1024 when completing its transfer
  MINUNIT_CHECK(XFERCFG::GetXFERCOUNT(dma_registers->CHANNEL[0].XFERCFG) == 1024);
  MINUNIT_CHECK(XFERCFG::GetXFERCOUNT(dma_registers->CHANNEL[1].XFERCFG) == 1024);
  MINUNIT_CHECK(std::equal(inputBuffer.begin(), inputBuffer.end(), outputBuffer.begin()) == true);
}

...
MINUNIT_ADD(LPC812M101CppSpiSyncRxTx, LPC812M101CppSetupSpiSync, LPC812M101Teardown) {
  ...
  spiSyncPeripheral.Transceive(ChipEnables::kNone, testDataSend, testDataReceive, 8, true);
  minUnitCheck((testDataSend[0] & 0xFF) == testDataReceive[0]);
  // test "multi"(above 17 bits) transaction to test multi transfers
  testDataReceive.fill(0x0000u);
  spiSyncPeripheral.Transceive(ChipEnables::kNone, testDataSend, testDataReceive, 24, true);
  ...
  spiSyncPeripheral.Receive(ChipEnables::kNone, testDataReceive, 12, true);
